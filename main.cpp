...
int main() {
    ...
    s.omega = Eigen::Vector3d(1.0, 0.0, 0.5);       // <- set to (0,0,0.5) here to
                                                      //    add yaw rotation on top of
                                                      //    free fall; no other code
                                                      //    needs to change, since
                                                      //    dynamics.cpp integrates
                                                      //    orientation independently
                                                      //    of translation already.
    ...
    for (int i = 0; i < N; ++i) {
        ...
        double thrust_mag = 0.0 * mass * g0; // hover thrust: exactly cancels gravity when level

        Eigen::Vector3d F_gravity = gravity(mass);
        Eigen::Vector3d F_thrust_world = bodyToWorld(s.q, thrustBody(thrust_mag));
        Eigen::Vector3d F_total(0, 0, 0); // = F_gravity + F_thrust_world;
        Eigen::Vector3d T_total(0, 0, 0); // Nm
        ...
        if ((i + 1) % steps_per_second == 0) {
            ...
            std::cout << "t = " << t_now << " s\n";
            std::cout << "RKE = " << 0.5 * s.omega.dot((test_drone.inertia * s.omega)) << " J\n";
            std::cout << "L  = " << (test_drone.inertia * s.omega).norm() << "\n";
